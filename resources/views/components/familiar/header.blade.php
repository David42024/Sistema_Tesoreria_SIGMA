...
>
  ...
  >
    ...
    >
      ...
      <a href="{{ route('principal') }}" class="lg:hidden">
        ...
        <form id="seleccion-alumno" method="POST" action="{{ route('principal') }}">
          ...
          <div class="relative">
            ...
            <select
              id="search-button"
              name="idalumno"
              class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-200 bg-transparent py-2.5 pr-14 pl-4 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden xl:w-[430px] dark:border-gray-800 dark:bg-gray-900 dark:bg-white/[0.03] dark:text-white/90 dark:placeholder:text-white/30" x-on:change="document.querySelector('#seleccion-alumno').submit()"
            >
              <option value="0" hidden>No hay alumno seleccionado.</option>

              @foreach ($page->alumnos as $alumno)
                <option class="text-gray-800 dark:text-gray-200 bg-white dark:bg-gray-800" value="{{ $alumno['id_alumno'] }}"
                @if($page->alumnoSeleccionado != null)
                @if($alumno['id_alumno'] == $page->alumnoSeleccionado->getKey()) selected @endif
                @endif
                >{{ $alumno['apellido_paterno'] . " " . $alumno['apellido_materno'] . " " . $alumno['primer_nombre'] . " " . $alumno['otros_nombres'] . " | DNI: " . $alumno['dni'] }}</option>
              @endforeach
            </select>
          </div>
        </form>
